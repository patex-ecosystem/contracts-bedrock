...
import { DepositTx } from '@eth-patex/core-utils'
...
task('deposit', 'Deposits funds onto L2.')
  ...
  .setAction(async (args, hre) => {
    ...
    const proxy = await hre.deployments.get('PatexPortalProxy')

    const PatexPortal = await hre.ethers.getContractAt(
      'PatexPortal',
      proxy.address
    )
    ...
    const tx = await PatexPortal.depositTransaction(
      to,
      amountWei,
      '3000000',
      false,
