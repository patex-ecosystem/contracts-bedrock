...
const check = {
  ...
  // - check version
  PatexMintableERC20Factory: async (
    hre: HardhatRuntimeEnvironment,
    signer: Signer
  ) => {
    const PatexMintableERC20Factory = await hre.ethers.getContractAt(
      'PatexMintableERC20Factory',
    ...
    const BRIDGE = await PatexMintableERC20Factory.BRIDGE()
    assert(BRIDGE !== hre.ethers.constants.AddressZero)

    await checkProxy(hre, 'PatexMintableERC20Factory', signer.provider)
    await assertProxy(hre, 'PatexMintableERC20Factory', signer.provider)
  },
  // L1BlockNumber
  // - check version
  ...
  // - check version
  PatexMintableERC721Factory: async (
    hre: HardhatRuntimeEnvironment,
    signer: Signer
  ) => {
    ...
    const BRIDGE = await PatexMintableERC721Factory.BRIDGE()
    assert(BRIDGE !== hre.ethers.constants.AddressZero)
    console.log(`  - BRIDGE: ${BRIDGE}`)

    const REMOTE_CHAIN_ID =
      await PatexMintableERC721Factory.REMOTE_CHAIN_ID()
    assert(REMOTE_CHAIN_ID !== 0)
    console.log(`  - REMOTE_CHAIN_ID: ${REMOTE_CHAIN_ID}`)

    await checkProxy(hre, 'PatexMintableERC721Factory', signer.provider)
    await assertProxy(hre, 'PatexMintableERC721Factory', signer.provider)
  },
  // ProxyAdmin
  // - check owner
