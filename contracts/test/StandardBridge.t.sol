...
} from "../universal/PatexMintableERC20.sol";
...
contract StandardBridgeTester is StandardBridge {
    ...
    function isPatexMintableERC20(address _token) external view returns (bool) {
        return _isPatexMintableERC20(_token);
    }
 ...
 * @notice Simple implementation of the legacy PatexMintableERC20.
 */
contract LegacyMintable is ERC20, ILegacyMintableERC20 {
     ...
     *         it is how the actual legacy patex mintable token does the
...
contract StandardBridge_Stateless_Test is CommonTest {
    ...
    PatexMintableERC20 internal mintable;
    ...
    function setUp() public override {
        ...
        mintable = new PatexMintableERC20({
            _bridge: address(0),
    ...
    /**
     * @notice Test coverage for identifying PatexMintableERC20 tokens.
     *         This function should return true for both modern and legacy
     *         PatexMintableERC20 tokens and false for any accounts that
     *         do not implement the interface.
     */
    function test_isPatexMintableERC20_succeeds() external {
        // Both the modern and legacy mintable tokens should return true
        assertTrue(bridge.isPatexMintableERC20(address(mintable)));
        assertTrue(bridge.isPatexMintableERC20(address(legacy)));
        // A regular ERC20 should return false
        assertFalse(bridge.isPatexMintableERC20(address(erc20)));
        // Non existent contracts should return false and not revert
        assertEq(address(0x20).code.length, 0);
        assertFalse(bridge.isPatexMintableERC20(address(0x20)));
    }
