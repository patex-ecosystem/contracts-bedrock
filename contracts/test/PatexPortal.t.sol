...
contract PatexPortal_Test is Portal_Initializer {
    ...
    /**
     * @notice The PatexPortal can be paused by the GUARDIAN
     */
    ...
    /**
     * @notice The PatexPortal reverts when an account that is not the
    ...
    function test_pause_onlyGuardian_reverts() external {
        ...
        vm.expectRevert("PatexPortal: only guardian can pause");
        vm.prank(alice);
        op.pause();
    ...
    /**
     * @notice The PatexPortal can be unpaused by the GUARDIAN
     */
    ...
    /**
     * @notice The PatexPortal reverts when an account that is not
    ...
    function test_unpause_onlyGuardian_reverts() external {
        ...
        assertTrue(op.GUARDIAN() != alice);
        vm.expectRevert("PatexPortal: only guardian can unpause");
        vm.prank(alice);
        op.unpause();
    ...
    function test_depositTransaction_contractCreation_reverts() external {
        ...
        vm.expectRevert("PatexPortal: must send to address(0) when creating a contract");
    ...
    function test_depositTransaction_smallGasLimit_reverts() external {
        vm.expectRevert("PatexPortal: gas limit must cover instrinsic gas cost");
...
contract PatexPortal_FinalizeWithdrawal_Test is Portal_Initializer {
    ...
    function callPortalAndExpectRevert() external payable {
        vm.expectRevert("PatexPortal: can only trigger one withdrawal per transaction");
    ...
    function test_proveWithdrawalTransaction_paused_reverts() external {
        ...
        vm.expectRevert("PatexPortal: paused");
    ...
    // Test: proveWithdrawalTransaction cannot prove a withdrawal with itself (the PatexPortal) as the target.
    function test_proveWithdrawalTransaction_onSelfCall_reverts() external {
        _defaultTx.target = address(op);
        vm.expectRevert("PatexPortal: you cannot send messages to the portal contract");
        op.proveWithdrawalTransaction(
            _defaultTx,
    ...
    function test_proveWithdrawalTransaction_onInvalidOutputRootProof_reverts() external {
        ...
        vm.expectRevert("PatexPortal: invalid output root proof");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProve_reverts() external {
        ...
        vm.expectRevert("PatexPortal: withdrawal hash has already been proven");
        op.proveWithdrawalTransaction(
            _defaultTx,
            _proposedOutputIndex,
            _outputRootProof,
    ...
    function test_proveWithdrawalTransaction_replayProveChangedOutputRoot_succeeds() external {
        ...
        assembly {
            ...
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in the PatexPortal
        ...
        external
    {
        ...
        assembly {
            mstore(0x00, sload(_withdrawalHash.slot))
            mstore(0x20, 52) // 52 is the slot of the `provenWithdrawals` mapping in PatexPortal
            slot := keccak256(0x00, 0x40)
        }
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalNotProven_reverts() external {
        ...
        vm.expectRevert("PatexPortal: withdrawal has not been proven yet");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifWithdrawalProofNotOldEnough_reverts() external {
        ...
        vm.expectRevert("PatexPortal: proven withdrawal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_ifOutputTimestampIsNotFinalized_reverts() external {
        ...
        // Attempt to finalize the withdrawal
        vm.expectRevert("PatexPortal: output proposal finalization period has not elapsed");
        op.finalizeWithdrawalTransaction(_defaultTx);
    ...
    function test_finalizeWithdrawalTransaction_onReplay_reverts() external {
        ...
        vm.expectRevert("PatexPortal: withdrawal has already been finalized");
        op.finalizeWithdrawalTransaction(_defaultTx);
    }
...
contract PatexPortalUpgradeable_Test is Portal_Initializer {
    ...
    function test_params_initValuesOnProxy_succeeds() external {
        PatexPortal p = PatexPortal(payable(address(proxy)));
    ...
    function test_initialize_cannotInitProxy_reverts() external {
        ...
        PatexPortal(payable(proxy)).initialize(false);
    }

    function test_initialize_cannotInitImpl_reverts() external {
        vm.expectRevert("Initializable: contract is already initialized");
        PatexPortal(opImpl).initialize(false);
    }
